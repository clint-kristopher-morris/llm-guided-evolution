# ['xXxWNoCq5hEnTDeFj9KOWCDyxUB', (0.934499979019165, 519574.0)]
...
class SE_LN(nn.Module):
    def __init__(self, cin, dropout_p=0.2):
        super().__init__()
        self.gavg = nn.AdaptiveAvgPool2d((1,1))  
...
def custom_min_pool2d(x, ks, ceil_mode):
    return -F.max_pool2d(-x, ks, ceil_mode=ceil_mode)
...
class FCT(nn.Module):
    def __init__(self, cin, cout):
        ...
        self.minpool = custom_min_pool2d
    ...
    def forward(self, x):
        x = torch.cat((
            self.maxpool(x),
            self.minpool(x, 2, ceil_mode=True),
            self.dw(x),
        ), 1)
        x = self.pw(x)
        x = self.bn(x)
...
class EVE(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pw(x)
        x = self.bn(x)
        x = self.activation(x) # Add ReLU activation function
        return x
# --OPTION--
import torch.nn as nn
...
class ME(nn.Module):
    def __init__(self, cin, cout):
        ...
        self.spatial_bn = nn.BatchNorm2d(cout, track_running_stats=True, affine=True) # Add spatial batch normalization
    ...
    def forward(self, x):
        ...
        x = self.spatial_bn(x) # Apply spatial batch normalization
        x = self.dropout(x)
        return x
# --OPTION--
def pad_num_y(k_s):
