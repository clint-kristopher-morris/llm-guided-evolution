...
class SE_LN(nn.Module):
    def __init__(self, cin, dropout=0.2):
        super().__init__()
        self.dropout = nn.Dropout(dropout)
        self.gavg = nn.AdaptiveAvgPool2d((1, 1))
        self.ln = nn.LayerNorm(cin)
        self.act = nn.SiLU()  # Use SiLU instead of Sigmoid for better performance
    ...
    def forward(self, x):
        ...
        x = self.ln(x)
        x = self.act(x)
        x = self.dropout(x)  # Add dropout to mitigate overfitting
        x = x.view(-1, x.size(1), 1, 1)
        return residual + x
# --OPTION--
