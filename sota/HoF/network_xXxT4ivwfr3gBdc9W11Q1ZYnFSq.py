# ['xXxT4ivwfr3gBdc9W11Q1ZYnFSq', (0.9258999824523926, 294994.0)]
...
class ConstantMomentumScheduler:
    def __init__(self, optimizer, T_max, final_momentum):
        self.optimizer = optimizer
        self.T_max = T_max
        self.final_momentum = final_momentum
        self.momentum_factor = 1 - (1 - self.final_momentum) / self.T_max

    def step(self, epoch):
        if epoch < self.T_max:
            self.optimizer.momentum *= self.momentum_factor
...
class SE(nn.Module):
    def __init__(self, cin, ratio, dropout_p=0.2):
        super().__init__()
        self.gavg = nn.AdaptiveAvgPool2d((1,1))
        self.fc1 = nn.Conv1d(cin, int(cin/ratio), kernel_size=1, bias=False)
        self.act1 = nn.ReLU()
        self.dropout = nn.Dropout(dropout_p)
        self.fc2 = nn.Conv1d(int(cin/ratio), cin, kernel_size=1, bias=False)
        self.act2 = nn.Sigmoid()

    def forward(self, x):
        y = x
        x = self.gavg(x)
        x = self.fc1(x.squeeze(-1).transpose(-1, -2)).transpose(-1, -2)
        x = self.act1(x)
        x = self.dropout(x)
        x = self.fc2(x)
        x = self.act2(x)
...
class EVE(nn.Module):
    def __init__(self, cin, cout):
        ...
        self.pw = nn.Conv2d(cin*2, cout, 1, 1, bias=False)
        self.bn = nn.BatchNorm2d(cout) # Add 1
        self.dropout = nn.Dropout2d(0.25) # Add 2
